...
p1 <- so2_ts |> 
  ...
  ACF(SO2_diff, lag_max = 28) |> 
  autoplot() + 
  scale_x_continuous(breaks = seq(0, 28, by = 7)) +
  geom_vline(xintercept = c(7, 14, 21, 28), linetype = "dotted", color = "red", alpha = 0.5) +
  labs(title = "Estación Guamaní (Lag 7)", x = NULL) + # Quitamos x para no repetir
  theme_minimal()
...
so2_ts |> 
  ...
  geom_vline(xintercept = c(7, 14, 21, 28), linetype = "dotted", color = "red", alpha = 0.5) +
  labs(
    title = "Autocorrelación (ACF): Resto de Estaciones",
    subtitle = "Doble diferenciación (lag=1 y lag=7)",
    y = "ACF",
    x = "Retardo (Días)"
  ) +
  
  facet_wrap(vars(ESTACION), ncol = 2, scales = "fixed") + 
  theme_light() +
  theme(
    legend.position = "none",
    strip.background = element_rect(fill = "#2C3E50"), # Un color más corporativo/oscuro para los encabezados
...
El análisis de residuos al cuadrado muestra que la varianza del error no es constante. Existen periodos de alta volatilidad (agrupamiento), especialmente visibles en la estación **Tumbaco**. Además, se observan valores atípicos extremos al inicio de los registros en Belisario y Carapungo.
