...
so2_ts <- so2_ts |> 
  mutate(SO2 = case_when(
      ESTACION == "Carapungo" & year(FECHA) <= 2005 ~ NA_real_,
      ESTACION == "Cotocollao" & year(FECHA) <= 2006 ~ NA_real_,
      ESTACION == "El Camal" & FECHA<= as_date("2016-01-01")~ NA_real_,
      TRUE ~ SO2
    ))|> 
...
so2_ts|> filter(ESTACION=="El Camal")|>autoplot(SO2)

```

Con la frecuencia diaria, pues, el SO2 al ser una sustancia muy volatil, es de interés saber los días que se alcanzan los máximos durante el año. Además que el SO2 suele estar ligado a la actividad humana, entonces, la medición diaria del SO2 tiene la capacidad de mostrar patrones cruciales.
...
tabla_ordenes_2 <- ordenes_diff_st |> 
  filter(ESTACION %in% vector_estaciones)
...
```{r, echo=FALSE, warning=FALSE}
estaciones_d1 <- tabla_ordenes |>
  filter(ndiffs==1) |>
  pull(ESTACION)

estaciones_d0<- tabla_ordenes |>
  filter(ndiffs==0) |>
  pull(ESTACION)

so2_ts |> 
  filter(ESTACION %in% estaciones_d1) |> 
...
library(patchwork) 
p1 <- so2_ts |> 
  filter(ESTACION %in% estaciones_d0) |> 
  ACF(SO2) |> 
  autoplot() + 
  labs(title = "ACF de cantidad de SO2") +
...
so2_ts |> 
  ...
  ACF(SO2_diff) |> 
  autoplot() + 
  labs(title = "ACF de cantidad de SO2 en estaciones diferenciadas") +
  facet_wrap(vars(ESTACION), scales = "fixed", ncol = 2) +
  theme(legend.position = "none")
...
so2_ts |> 
  ...
  PACF(SO2) |> 
  autoplot() + 
  labs(title = "PACF de cantidad de SO2 en estación Guamani") +
  facet_wrap(vars(ESTACION), scales = "fixed", ncol = 2) +
  theme(legend.position = "none")
...
so2_ts |> 
  ...
  PACF(SO2_diff) |> 
  autoplot() + 
  labs(title = "PACF de cantidad de SO2 por estaciones diferenciadas") +
  facet_wrap(vars(ESTACION), scales = "fixed", ncol = 2) +
  theme(legend.position = "none")
...
separar_datos_entrenamiento_prueba <- function(tsibble_datos, prop_entrenamiento = 0.8) {
  ...
  datos_ordenados <- tsibble_datos |>
    arrange(ESTACION, FECHA) 
  
  datos_entrenamiento <- datos_ordenados |>
    group_by(ESTACION) |>
    mutate(fila_corte = floor(n() * prop_entrenamiento)) |>
    slice(1:unique(fila_corte)) |> 
    select(-fila_corte) |> 
    ungroup()
  
  # 3. Definimos prueba tomando desde N+1 hasta el final
  datos_prueba <- datos_ordenados |>
    group_by(ESTACION) |>
    mutate(fila_corte = floor(n() * prop_entrenamiento)) |>
    slice((unique(fila_corte) + 1):n()) |> # Toma desde el corte+1 hasta el final
    select(-fila_corte) |>
    ungroup()
...
fit_camal<- datos_separados$entrenamiento |>
  filter(ESTACION == "El Camal") |>
  model(
    Modelo_Propuesto = ARIMA(SO2 ~ pdq(1, 0, 2) + PDQ(0, 0, 0, period=7)),
    Modelo_Automatico = ARIMA(SO2),
    Modelo_SNAIVE    = SNAIVE(SO2 ~ lag(7)),
    Modelo_Lineal   = TSLM(SO2 ~ trend() + season(period=7))
  )
...
glance_camal <- glance(fit_camal)

tabla_comparativa_1 <- bind_rows(glance_guamani, glance_resto,glance_camal) |>
...
```{r, echo=FALSE}
tabla_comparativa_2 <- accuracy(bind_rows(fit_guamani, fit_resto,fit_camal)) |>
...
```{r, echo=FALSE}
todos_los_modelos <- bind_rows(fit_guamani, fit_resto,fit_camal) 
...
coeficientes_mle <- bind_rows(fit_guamani, fit_resto,fit_camal) |>
...
modelos_finales <- bind_rows(fit_guamani, fit_resto,fit_camal)
```
...
#Guamani
h<-7
test_guamani <- datos_separados$prueba |> filter(ESTACION == "Guamani")
h_guamani <- nrow(test_guamani) + h

fc_guamani <- fit_guamani |>
  forecast(h = h_guamani)

inicio_zoom_guamani <- min(test_guamani$FECHA)-months(2)

fc_guamani |>
  autoplot(
    datos_separados$entrenamiento |> 
      filter(ESTACION == "Guamani", FECHA >= inicio_zoom_guamani),
    level = NULL
  ) +
  autolayer(test_guamani, SO2, colour = "black") +
  geom_vline(xintercept = as.numeric(min(test_guamani$FECHA)), linetype="dotted", alpha=0.5) +
  labs(
    title = "Guamaní: Detalle de Transición y Pronóstico",
    subtitle = "Zoom: Últimos 2 meses de entrenamiento + Test + Futuro",
    y = expression(paste("Concentración ", SO[2], " (", mu, "g/m"^3, ")"))
  ) 
```
...
#El Camal
h<-7
test_camal <- datos_separados$prueba |> filter(ESTACION == "El Camal")
h_camal <- nrow(test_camal) + h

fc_camal <- fit_camal |>
  forecast(h = h_camal)

inicio_zoom_camal <- min(test_camal$FECHA)-months(2)

fc_camal |>
  autoplot(
    datos_separados$entrenamiento |> 
      filter(ESTACION == "El Camal", FECHA >= inicio_zoom_camal),
    level = NULL
  ) +
  autolayer(test_camal, SO2, colour = "black") +
  geom_vline(xintercept = as.numeric(min(test_camal$FECHA)), linetype="dotted", alpha=0.5) +
  labs(
    title = "El Camal: Detalle de Transición y Pronóstico",
    subtitle = "Zoom: Últimos 2 meses de entrenamiento + Test + Futuro",
    y = expression(paste("Concentración ", SO[2], " (", mu, "g/m"^3, ")"))
  ) 
```
...
```{r,echo=FALSE, warning=FALSE}
# Camal
  datos_train_resto <- datos_separados$entrenamiento |>
    filter(!ESTACION %in% estaciones_d0)
  
  datos_test_resto <- datos_separados$prueba |>
    filter(!ESTACION %in% estaciones_d0)
  
  
fit_resto <- datos_train_resto |>
  model(
    Modelo_Propuesto  = ARIMA(SO2 ~ pdq(0, 1, 2) + PDQ(0, 0, 0, period = 7)),
    Modelo_Automatico = ARIMA(SO2),
    Modelo_SNAIVE     = SNAIVE(SO2 ~ lag(7)),
  ...
  # Calculamos el horizonte necesario: Máxima longitud del test + 7 días
  h_resto <- datos_test_resto |> count(ESTACION) |> pull(n) |> max() + 7
  
  fc_resto <- fit_resto |>
    forecast(h = h_resto)
  
  # --- 4. Gráfico con Zoom y Facetas ---
  
  fecha_inicio_zoom <- min(datos_test_resto$FECHA) - months(2)
  
  lista_estaciones <- unique(fc_resto$ESTACION)
  
  # 2. Generar un gráfico por cada estación
  for (nombre_estacion in lista_estaciones) {
    
    # A. Filtrar datos específicos para esta estación
    fc_actual    <- fc_resto |> filter(ESTACION == nombre_estacion)
    test_actual  <- datos_test_resto |> filter(ESTACION == nombre_estacion)
    train_actual <- datos_train_resto |> filter(ESTACION == nombre_estacion)
    
    # B. Definir Zoom (3 meses antes del inicio del test de ESTA estación)
    fecha_zoom <- min(test_actual$FECHA) - months(3)
    
    # C. Crear el gráfico
    p <- fc_actual |>
      autoplot(
        train_actual |> filter(FECHA >= fecha_zoom),
        level = NULL
      ) +
      autolayer(test_actual, SO2, colour = "black") +
      geom_vline(xintercept = as.numeric(min(test_actual$FECHA)), 
                 linetype = "dotted", alpha = 0.5) +
      labs(
        title = paste("Estación:", nombre_estacion),
        subtitle = "Validación y Pronóstico (Zoom 2 meses)",
        y = expression(paste("Concentración ", SO[2], " (", mu, "g/m"^3, ")"))
      ) +
      theme_minimal()
    
    # D. Imprimir el gráfico (Necesario dentro de un bucle)
    print(p)
  }
```
...
```{r,echo=FALSE, warning=FALSE}
todos_modelos <- bind_rows(fit_guamani, fit_resto,fit_camal)
...
```{r,echo=FALSE, warning=FALSE}
coeficientes_propuestos <- bind_rows(fit_guamani, fit_resto,fit_camal) |>
