...
so2_ts <- so2_ts |> 
  mutate(SO2 = ifelse(ESTACION == "Carapungo", 
                    ifelse(year(FECHA) > 2005, SO2, NA), 
                    SO2)) |> 
  mutate(SO2 = ifelse(ESTACION == "El Camal", 
                    ifelse(year(FECHA) > 2014, NA, SO2), 
                    SO2)) |> 
  mutate(SO2 = ifelse(ESTACION == "Cotocollao", 
                    ifelse(year(FECHA) > 2006, SO2, NA), 
                    SO2))
...
p1 <- so2_ts |> 
  ...
  ACF(SO2) |> 
  autoplot() + 
  labs(title = "ACF de cantidad de SO2 en estación Guamani") +
...
fc1_guamani |>
  ...
  labs(
    ...
    title = "Pronósticos de concentración de SO2 en Guamaní"
  ) +
...
fc1_Camal |> 
  ...
  autoplot(so2_ts |> 
             filter(ESTACION == "El Camal", 
                    ...
                    yearmonth(FECHA) <= yearmonth("2017-04")),
           level = NULL
  ) +
  labs(
    x = "Time", 
